...
describe('cart store', () => {
  beforeEach(() => {
    const { set } = (useCartStore as any)
    set({ items: [], totals: { subtotal: 0, discount: 0, total: 0, promotionText: null } })
  })
  ...
  it('computes totals from state', () => {
    const { set } = (useCartStore as any)
    set({ totals: { subtotal: 100, discount: 10, total: 90, promotionText: 'Promo' } })
